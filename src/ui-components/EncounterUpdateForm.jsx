...
export default function EncounterUpdateForm(props) {
  const {
    id: idProp,
    encounter: encounterModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = encounterRecord
      ? { ...initialValues, ...encounterRecord }
      : initialValues;
    setWorkDate(cleanValues.workDate);
    setClinicName(cleanValues.clinicName);
    setRole(cleanValues.role);
    setSessionType(cleanValues.sessionType);
    setMeetingType(cleanValues.meetingType);
    setOrgTime(cleanValues.orgTime);
    setGamePlanTime(cleanValues.gamePlanTime);
    setNotes(cleanValues.notes);
    setErrors({});
  };
  const [encounterRecord, setEncounterRecord] =
    React.useState(encounterModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getEncounter.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getEncounter
        : encounterModelProp;
      setEncounterRecord(record);
    };
    queryData();
  }, [idProp, encounterModelProp]);
  React.useEffect(resetStateValues, [encounterRecord]);
  const validations = {
    workDate: [],
    clinicName: [],
    role: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          orgTime: orgTime ?? null,
          gamePlanTime: gamePlanTime ?? null,
          notes: notes ?? null,
        };
        ...
        try {
          ...
          await client.graphql({
            query: updateEncounter.replaceAll("__typename", ""),
            variables: {
              input: {
      ...
      {...getOverrideProps(overrides, "EncounterUpdateForm")}
      {...rest}
    >
      <TextField
        label="Work date"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || encounterModelProp)}
